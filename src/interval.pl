...
constant @ X::A:B <=> number(X) | A = X, B = X.
...
compute(C, Expr) :-
        ...
        Expr =.. [F|Args],
        maplist(compute, Args1, Args),
        compute(C, F, Args1).

compute(_, _, As) :-
        member(As, [[inf, ninf], [ninf, inf]]),
        !,
        throw(domain_error(compute/3)).
compute(inf, _, [C, inf]) :- !.
compute(inf, _, [inf, _]):- !.
compute(ninf, _, [ninf, _]):- !.
compute(ninf, _, [_, ninf]):- !.
