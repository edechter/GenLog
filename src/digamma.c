...
foreign_t
pl_digamma(term_t in, term_t out)
{ double x, y;
  int rval;

  if ( !PL_get_float(in, &x) )
    return PL_warning("digamma/2: instantiation fault");\
  y = digamma(x);
    
  rval = PL_unify_float(out, y);
...
install_t
install()
{ PL_register_foreign("digamma", 2, pl_digamma, 0);
